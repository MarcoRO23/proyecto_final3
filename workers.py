...
def procesar_municipios(df, entidades, worker_id):
    ...
    subset = df[df["ID_ENTIDAD"].isin(entidades)]
    resultado = subset.groupby(["ID_MUNICIPIO", "ID_ENTIDAD"]).size().reset_index(name="total_accidentes")
...
def procesar_victimas(df, entidades, worker_id):
    ...
    subset = df[df["ID_ENTIDAD"].isin(entidades)]
    resultado = subset.groupby("ID_ENTIDAD").agg({
        "TOTAL_MUERTOS": "sum",
        "TOTAL_HERIDOS": "sum"
    }).reset_index()
    resultado["TOTAL_VICTIMAS"] = resultado["TOTAL_MUERTOS"] + resultado["TOTAL_HERIDOS"]
    return {"worker": worker_id, "data": resultado.to_dict(orient="records")}
